...
const TrainingVideo: React.FC = () => {
  ...
  <motion.div
    className="bg-[#F6F5FA] px-4 sm:px-6 lg:px-[37px]"
  ...
  <div
    className="bg-[#F6F5FA] w-full mx-auto sm:max-w-full md:max-w-[90%] xl:max-w-[1469px] 
               sm:px-4 lg:px-6 mb-[40px]"
  >
    {/* Header */}
      ...
      <h1 className="flex items-center gap-2 text-midGray text-[16px] leading-[16px] font-[500]">
        <span className="bg-[#DAF1FB] rounded-full w-[40px] h-[40px] flex items-center justify-center">
          <img
            src={videoicon}
            alt="Section icon"
            className="w-5.5 h-5.5 filter-blue"
          />
        </span>
        Training Videos
      </h1>
    ...
    >
      <div
        className="w-full h-auto mt-[35px] shadow-[0px_0px_2px_0px_#00000040] 
                   bg-[#FCFCFC] pt-[50px] pb-[50px] flex flex-col gap-[40px] rounded-[10px]"
      >
        <h1 className="text-[24px] font-bold pl-[30px] text-[#4D4D4D]">
          Training Videos
        </h1>
        ...
        {/* Video Cards */}
        <div className="flex flex-wrap items-center justify-center gap-[45px] px-4">
          {videos.map((video) => (
            <VideoCard
              key={video.id}
              videos={video}
